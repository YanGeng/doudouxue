...
export const useBattery = () => {
	...
	onMounted(async () => {
		...
		// 电池充电状态更新时被调用
		BatteryManager.onchargingchange = ({ target }: Event) => {
			updateBattery(target as unknown as Battery);
			console.log(target, '电池充电状态改变了');
		};
		// 电池充电时间更新时被调用
		BatteryManager.onchargingtimechange = ({ target }: Event) => {
			updateBattery(target as unknown as Battery);
			console.log(target, '电池充电了');
		};
		// 电池断开充电时间更新时被调用
		BatteryManager.ondischargingtimechange = ({ target }: Event) => {
			updateBattery(target as unknown as Battery);
			console.log(target, '电池断开充电了');
		};
		// 电池电量更新时被调用
		BatteryManager.onlevelchange = ({ target }: Event) => {
			updateBattery(target as unknown as Battery);

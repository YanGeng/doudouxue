...
import type { Battery } from '@/libs/hooks/useBattery';
...
.huawei-charge {
	...
	@for $i from 1 through 15 {
		...
		li:nth-child(#{$i}) {
			$width: round(15 + random() * 15) * 1px;

			top: 50%;
			left: round(random() * 70) * 1px;
			width: $width;
			height: $width;
			transform: translate(-50%, -50%);
			animation: move-to-top (round(random() * 6) + 3) s ease-in-out -(random() * 5) s infinite;
		}
	}

<template>
	<view class="user-area">
		<view class="header-area padding-lr-sm" :class="(is_mp && !is_alipay) ? 'padding-top-big' : 'padding-top'">
			<view class="dflex-b">
				<!-- <view class="member-area padding-top-sm margin-bottom dflex pos-r" @click="to('/pages_user/user/setting/personal')"> -->
				<view class="member-area padding-top-sm margin-bottom dflex pos-r">
					<view>
						<image class="headimg border-radius-c" :src="member.member_headimg || '/static/images/user/default_head.jpg'"></image>
					</view>
					<view class="margin-left-sm">
						<view class="info-box">
							<text class="fs-lg">{{ member.member_nickname || member.member_name || '严选' }}</text>
						</view>
						<view><text class="fs-xxs">{{ member.member_city }}</text></view>
					</view>
				</view>
				<!-- <view class="border-radius-big bg-base dflex-c padding-lr" @click="to('/pages_user/user/integral/sign')">
			...
			<view class="border-radius">
				<view class="vip-card-area pos-r padding-lr padding-tb-sm">
					<view>
						...
						<text class="margin-left-sm">豆豆学 会员</text>
					</view>
				</view>

				<!-- <view class="stats-area dflex-c bg-main">
					<view class="item dflex dflex-flow-c" @click="toOrder('/pages_user/user/order/order', '全部')">
						<text class="num">{{ member.member_monetary / 100 || 0 }}</text>
						<text>累计消费</text>
						<view class="vertical-line"></view>
					</view>
					<view class="item dflex dflex-flow-c" @click="to('/pages_user/user/coupon/coupon')">
		...
		<view class="container-area padding-lr-sm padding-bottom-sm">
			...
			<!-- <view class="border-radius margin-top-sm bg-main">
				<use-list-title title="我的订单" iconfont="icondingdan" color="#ff6a6c" fwt="600" tip="查看全部订单"
					@goto="toOrder('/pages_user/user/order/order', '全部')"></use-list-title>

				<view class="order-area padding-bottom-sm padding-lr dflex-c">
					<view class="item dflex dflex-flow-c" @click="toOrder('/pages_user/user/order/order', '待付款')">
					...
					<view class="item dflex dflex-flow-c" @click="toOrder('/pages_user/user/order/order', '待发货')">
						<view class="iconfont">&#xe6d9;<view class="badge badge-small"
					...
					<view class="item dflex dflex-flow-c" @click="toOrder('/pages_user/user/order/order', '待收货')">
						<view class="iconfont">&#xe6d7;<view class="badge badge-small"
					...
					<view class="item dflex dflex-flow-c" @click="toOrder('/pages_user/user/order/order', '待评价')">
						<view class="iconfont">&#xe6db;<view class="badge badge-small"
					...
					<view class="item dflex dflex-flow-c" @click="toOrder('/pages_user/user/order/order', '售后中')">
						<view class="iconfont">&#xe715;<view class="badge badge-small"
			...
			<view class="border-radius margin-top-sm bg-main">
				...
				<use-list-title title="我的足迹" iconfont="iconzuji" color="#ffab6c" fwt="600" :tip="stats.browsing"
					@goto="to('/pages_user/user/browsing/browsing')"></use-list-title>
				...
				<use-list-title title="我的收藏" iconfont="iconshoucang-" color="#ff6a6c" fwt="600" :tip="stats.collect"
					@goto="to('/pages_user/user/collect/collect')"></use-list-title>
				...
				<!-- <use-list-title title="分销中心" iconfont="iconyixiaoshou" color="#ffab6c" fwt="600" tip="分享赚钱"
					@goto="to('/pages_user/user/distribution/distribution')"></use-list-title> -->
				<!-- <use-list-title title="收货人" iconfont="icondizhi-" color="#5a9ded" fwt="600"
					@goto="to('/pages_user/user/address/address')"></use-list-title> -->
				<use-list-title title="设置" iconfont="iconshezhi-" color="#58bc8a" fwt="600" tip='默认身份“学生”，“个人资料”中可修改'
					@goto="to('/pages_user/user/setting/setting')"></use-list-title>
				<use-list-title title="工信部备案" iconfont="iconquanbudingdan-" color="#a09a9a" fwt="600" tip="沪ICP备2025112865号-1A"></use-list-title>
				<use-list-title v-if="isAdmin" title="管理员" iconfont="iconquanbudingdan-" color="#a09a9a" fwt="600"
					@goto="to('/pages_user/user/admin/admin')"></use-list-title>
			</view>
			...
			<view v-else class="border-radius margin-top-sm padding-sm dflex-c bg-main log-out-btn"
				@click="to('/pages/login/login-new')">
...
<script>
	...
	export default {
		computed: {
			...mapGetters(['islogin', 'isLoggedIn', 'member', 'user_role', 'isPreLoginSucess'])
		},
		data() {
			return {
				isreq: false,
		...
		onShow() {
			console.log('this member', this.islogin, this.member, this.user_role, this.isPreLoginSucess);
			let _this = this;
			if (!this.islogin) {
				this.$api.msg('账号未登录');
				return;
			...
			uni.removeTabBarBadge({
			                index: 1, // TabBar 索引，根据实际情况修改
			                success: function () {
			                    console.log('TabBar 数字小上标已移除');
			                },
			                fail: function (err) {
			                    console.error('移除 TabBar 数字小上标失败', err);
			                }
			            });

...
export default {
	...
	onShow() {
		console.log('city.vue:', this.activeCity, this.current_city, this.location_city);
	},
	onLoad() {
		//动态更新数据
		// setTimeout(() => {
			...
			//修改当前城市
			this.location_city = uni.getStorageSync('location_city');
			if (this.location_city) {
				this.activeCity = this.location_city;
			} else {
				this.activeCity = {
					cityName: '上海市',
					cityCode: 310100
			...
			console.log('city.vue:', this.activeCity)
	...
	methods: {
		...
		cityClick(item) {
			console.log("this.current_city bf", this.location_city);
			let _this = this;
			console.log("item ", item);
			this.updateCurrentCity(item.cityName);
			// _this.updateLocationCity(item.activateCity);
			console.log("this.current_city af", item, this.location_city);

<template>
	<view class="content bg-drak ft-main">
		...
		<view class="row dflex-c border-line padding-lr">
		...
		<view class="row dflex border-line padding-lr">
			<text class="tit2">价格 (￥)</text>
		</view>
		<view>
			<slider :value="goods.price/100" @change="priceSliderChange" min="50" max="500" step="10" show-value />
		</view>

		<view v-if="user_role != 'student'">
			<view class="row dflex border-line padding-lr">
				<text class="tit">感谢费 (￥) </text>
				<input class="input" type="number" v-model="goods.thanksMoney" placeholder="请输入感谢费"
					placeholder-class="placeholder" />
			</view>
			<view class="row dflex border-line padding-lr">
				<text class="tit2">感谢费详情</text>
			</view>
			<view class="uni-textarea dflex border-line padding-lr">
				<view>
					<textarea class="input" type="text" v-model="goods.thanksDetails" placeholder="老师接此单,需收取感谢费详情"
...
import {
	...
	export default {
		...
		onLoad(options) {
			...
			if (options.type === 'edit') {
				...
				this.$db['usemall-goods'].where({
					...
					if (res.code === 200) {
						console.log("load data", res, _this.selectKemu, _this.selectShoukeType);
						_this.goods = res.datas[0];
						// // 添加类目
						// // let selectCate = [];
						// if (_this.goods.cids) {
						// for (let cate of _this.goods.cids) {
						// 	for (let item of _this.rangeKemu) {
						// 		if (cate === item.text || cate === item.cid) {
						// 			// selectCate.push(item.value);
						// 			_this.selectKemu.push(item.value);
						// 		}
						// 	}
						// }
						// }
						...
						// 处理科目选择
						if (_this.goods.cids) {
							_this.selectKemu = _this.goods.cids.flatMap(cate => {
								const item = _this.rangeKemu.find(item => cate === item.text || cate === item.cid);
								return item ? [item.value] : [];
							});
						}

						// // 处理授课方式选择
						// if (_this.goods.shoukeType) {
						//   _this.selectShoukeType = _this.goods.shoukeType.flatMap(cate => {
						//     const item = _this.rangeShoukeType.find(item => cate === item.text);
						//     return item ? [item.value] : [];
						//   });
						// }
						// 处理授课方式选择
						if (_this.goods.shoukeType) {
							const newSelectShoukeType = [];
							for (let cate of _this.goods.shoukeType) {
								for (let item of _this.rangeShoukeType) {
									if (cate == item.text) {
										newSelectShoukeType.push(item.value);
									}
								}
							}
							_this.selectShoukeType = newSelectShoukeType; // 直接赋值触发响应式更新
						}

						// 使用nextTick确保DOM更新
						_this.$nextTick(() => {
							console.log('数据更新后:', _this.selectKemu, _this.selectShoukeType);
						});

						console.log('selectKemu', _this.selectKemu, this.selectShoukeType);
		...
		methods: {
			...
			changeKemu(e) {
				console.log('e:', this.selectKemu, e.detail.value);
				...
				if (this.type === 'edit') {
					await this.$func.usemall
						.call('goods/updateNewGoods', {
							...
							thanksMoney: this.goods.thanksMoney,
							thanksDetails: this.goods.thanksDetails
						})
						.then(res => {
